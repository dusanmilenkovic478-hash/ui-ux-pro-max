...
/* Chrome verwirft das Sprach-Objekt, sobald keine Referenz mehr darauf
   zeigt — dann passiert einfach nichts, ohne Fehlermeldung. Deshalb
   halten wir es hier fest, bis es fertig gesprochen ist. */
let laufendeRede = null;
let sprachVersuch = 0;

function vorlesen(text, ohneStimme) {
  const knopf = $("btn-hoeren");
  const feld = $("hoer-meldung");
  if (feld && !ohneStimme) feld.hidden = true;

  if (!window.speechSynthesis || typeof SpeechSynthesisUtterance === "undefined")
    return zeigeSprachProblem("fehlt");

  // Läuft gerade etwas? Dann ist der Knopf ein Stopp-Knopf.
  if (!ohneStimme && knopf && knopf.classList.contains("laeuft")) {
  ...
  // Beim zweiten Anlauf ohne feste Stimme: manche Browser sprechen nur
  // mit ihrer eigenen Vorauswahl.
  if (!ohneStimme) {
    const stimme = deutscheStimme || holeStimme();
    if (stimme) rede.voice = stimme;
  }

  laufendeRede = rede;                // <- verhindert das stille Verschlucken
  ...
  rede.onerror = (e) => {
    ...
    if (!ohneStimme) return vorlesen(text, true);      // zweiter Anlauf
  ...
  // Nichts passiert? Erst den zweiten Anlauf, dann erst aufgeben.
  // 1,6 s, weil die Sprachausgabe beim allerersten Mal länger braucht.
  setTimeout(() => {
    if (meinVersuch !== sprachVersuch) return;           // längst ein neuer Versuch
    if (startete || speechSynthesis.speaking || speechSynthesis.pending) return;
    if (!ohneStimme) return vorlesen(text, true);
...
function sprachDiagnose() {
  ...
  if (window.speechSynthesis)
    t += ` Status: spricht=${speechSynthesis.speaking}, wartet=${speechSynthesis.pending}, pausiert=${speechSynthesis.paused}.`;
  return t;
}
